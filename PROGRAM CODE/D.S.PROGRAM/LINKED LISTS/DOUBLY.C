...
void binsert(int num,int num1)
{
	...
	else
	{
		...
		else
		{
			...
			create->prev=temp->prev;
			create->next=temp->prev->next;
			create->data=num;
			temp->prev->next=create;
			temp->prev=create;
			printf("\n%d IS INSERT BEFORE %d PROPERRLY",num,num1);
		}
	}
...
void bdelete(int num)
{
	...
	else
	{
		...
		{
			temp1=temp->prev;
			temp->prev->prev->next=temp->prev->next;
			temp->prev=temp->prev->prev;
			printf("NODE BEFORE %d IS DELETED SUCCESSFULLY WHOSE DATA PART IS %d",temp->data,temp1->data);
			free(temp1);
		}
...
void edelete(int num)
{
	...
	else
	{
		...
		else
		{
			...
			printf("\nTHE NODE IS DELETED WHOSE DATA PART IS %d",temp->data);
			free(temp);
		}
	}
}
...
void fdisplay()
{
	...
	{
		printf("\nLINKLIST CONTAIN BELOW ELEMENT(FORWARD)::\n\n");
		printf("\t\t\t\t\t ");
		for(temp=head;temp!=NULL;temp=temp->next)
		{
			c++;
...
void rdisplay()
{
	...
	{
		printf("\nLINK LIST CONTAIN BELOW ELEMENT(BACKWARD)::\n\n");
		printf("\t\t\t\t\t ");
		for(temp=head;temp->next!=NULL;temp=temp->next);
