...
void linear(int,int,int arr[]);	// LOOK HOW ARRAY IS RECEIVED THROUGH FUNCTION
void binary(int,int,int arr[]);	// LOOK HOW ARRAY IS RECEIVED THROUGH FUNCTION
	...
	while(1)
	{
		...
		switch(n)
		{
			case 1:
				...
				linear(n,n1,arr);	// LOOK HOW ARRAY IS PASSED THROUGH FUNCTION
				break;
			...
			case 2:
				...
				binary(n,n1,arr);	// LOOK HOW ARRAY IS PASSED THROUGH FUNCTION
...
void linear(int n,int n1,int arr[])
{
	...
	if(flag!=1)	// BEST IDEA TO USE THE CONDITION
...
int binary_r(int n,int n1,int arr[])
{
